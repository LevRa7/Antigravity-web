    ...
    for (const port of OAUTH_FALLBACK_PORTS) {
        try {
            await new Promise((resolve, reject) => {
                ...
                test.listen(port, '0.0.0.0', () => test.close(resolve));
            });
            boundPort = port;
            break;
        } catch {
            ...
            res.end(`
                <html>
                  <body style="font-family:sans-serif;text-align:center;padding:60px 20px;background:#0d1117;color:#e6edf3">
                    ...
                    <p style="color:#8b949e">Авторизация успешна!</h1>
                    <p style="color:#8b949e">Google Аккаунт успешно добавлен. Перенаправление обратно...</p>
                    <script>
                      setTimeout(() => {
                        const host = window.location.hostname;
                        window.location.href = https:// + host + :9443/login;
                      }, 1500);
                    </script>
                  </body>
                </html>
...
async function writeToCredentialStore(token) {
    const tsSec = token.expiry_timestamp > 1e11 ? Math.floor(token.expiry_timestamp / 1000) : (token.expiry_timestamp || Math.floor(Date.now()/1000) + 3600);
    const expiry = new Date(tsSec * 1000)
        .toISOString()
        .replace(/\.(\d{3})Z$/, '.$1000Z');
